// lib/hike.js — cliente de Hike con reintentos ante errores transitorios (500/red).
...
// fetch con reintentos: reintenta ante errores de red o respuestas 5xx (culpa del servidor).
async function fetchConReintentos(url, opts = {}, intentos = 3, esperaMs = 2500) {
  let ultimoError = "";
  for (let i = 1; i <= intentos; i++) {
    try {
      const res = await fetch(url, opts);
      if (res.ok) return res;
      // 5xx = error del servidor de Hike -> vale la pena reintentar
      if (res.status >= 500 && i < intentos) {
        ultimoError = `HTTP ${res.status}`;
        await new Promise((r) => setTimeout(r, esperaMs * i));
        continue;
      }
      return res; // 4xx u otro: no reintentar, devolver tal cual
    } catch (e) {
      ultimoError = e?.message || String(e);
      if (i < intentos) await new Promise((r) => setTimeout(r, esperaMs * i));
    }
  }
  throw new Error(`Sin respuesta de Hike tras ${intentos} intentos (${ultimoError})`);
}
...
export async function obtenerAccessToken() {
  ...
  const res = await fetchConReintentos(TOKEN_URL, {
    method: "POST",
